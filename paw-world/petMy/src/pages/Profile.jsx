        ...
        <div className='mt-6'>
          ...
          {activeTab === 'media' && (
            <div className='mt-6 flex flex-wrap max-w-6xl gap-3'>
              {posts.flatMap((post) => // filter
                (post.image_urls || []).map((image, index) => (
                  <Link target='_blank' to={`/post/${post._id}`} key={`${post._id}-${index}`} className='block w-64'>
                    <img src={image} key={index}  className='w-full h-40 object-cover rounded-lg' alt='' />
