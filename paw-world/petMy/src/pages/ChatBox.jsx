  ...
  return user && (
    <div className='flex flex-col h-screen'>
      <div className="flex items-center gap-2 p-2 md:px-10 xl:pl-42 bg-linear-to-r from indigo-50 to-purple-50 border-b border-gray-300">
        <img src={user.profile_picture} alt="" className='w-10 h-10 rounded-full shadow' />
        <div>
        ...
        <div className="space-y-4 max-w-4xl mx-auto">
          {
            messages.toSorted((a,b) => new Date(a.createdAt) - new Date(b.createdAt)).map((message, index) =>(
              <div key={index} className={`flex flex-col ${message.to_user_id !== user._id ? 'items-start' : 'items-end'}`}>
                <div className={`p-3 rounded-lg max-w-xs ${message.to_user_id !== user._id ? 'bg-gray-200' : 'bg-indigo-500 text-white'}`}>
      ...
      <div className="px-4">
          <div className="flex items-center gap-3 pl-5 p-1.5 bg-white w-full max-w-xl mx-auto border border-gray-200 shadow rounded-full mb-5">
            ...
            <button onClick={sendMessage} className='bg-indigo-500 text-white px-4 py-1.5 rounded-full hover:bg-indigo-600 active:scale-95 transition'>
